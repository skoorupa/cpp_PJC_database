...
namespace parser {
    ...
    auto Parser::parse_node() -> std::unique_ptr<ast::Node> {
        ...
        switch (token_type) {
            ...
            case lexer::TokenType::DBExport: {
                auto arg = parse_call_single_arg();
                return std::make_unique<ast::DBExport>(((ast::StringLiteral*)arg.get())->getValue());
                break;
            }
            ...
            case lexer::TokenType::KMAddColumn: {
                auto args = parse_call_multiple_args();
                if (args.size() < 2)
                    throw std::string("!!! Parser error: need at least 2 arguments for add_column");
